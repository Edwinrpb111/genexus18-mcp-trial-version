...
Friction wishlist sweep — 26 items from a single popup-implementation session in `AcademicoHomolog1` distilled into permanent tool/playbook improvements. Headline: agents stop paying repeated pedágio for runtime-vs-design IDs, the popup-async gotcha, HTML format sanitization, and `For each` attribute writes inside WebPanel Events. A 3-angle code review of the result-state caught 6 real correctness bugs (sort, filter, timezone, flag-forwarding) — all fixed in this release.

...
Friction wishlist sweep — 50+ items from `docs/mcp-improvements-2026-05-22.md` distilled into permanent tool/playbook improvements. Headline: agents stop paying repeated pedágio for runtime-vs-design IDs, the popup-async gotcha, HTML format sanitization, `For each` attribute writes inside WebPanel Events, opaque build/patch envelopes, SQL-injection-shaped writes, hardcoded credentials, GAM misconfig, and master-page incompatibility — all surface in the same turn as the edit. Final wave (items 8/10/14/20/21/43/47/48/49/62/63/64/70/72/77/80/88) landed after a parallel multi-agent fan-out; one agent's worktree was lost to a socket error and recovered from the git stash.

### Wave 3 additions — 2026-05-23

- **Build envelope success/warning/error classification (item 10).** `genexus_lifecycle build wait_until_done=true` no longer wraps `"Build succeeded: 0 warnings, 0 errors"` in an `<e>error{…}</e>` MCP envelope. `LifecycleResponseShaper.ClassifyBuildOutcome` is the single source of truth: clean success → `isError=false`, `partial_success=true` → `warning` envelope, otherwise error. The original bug came from comparing `JobEntry.Status` against `"completed"` when the registry stamps `"succeeded"`.
- **Patch write-fallback false negatives (item 8).** `PatchService.ClassifyFallbackFailure` distinguishes `write_not_persisted` (retry-safe error) from `persisted_with_concurrent_change` (write OK but hash drifted post-write — now returns `Success` with a `postWriteHashDrift` warning and `RequiresReread` flag, not a generic error).
- **`code:` + `docUrl:` on every gotcha/lint (item 62).** `Helpers/GotchaCodes.cs` is the central registry; gotcha emit sites (`HtmlFormatGotcha`, `Spc0150Preflight`, `LayoutGotchaScanner`, `LintKbCharsetLossy`, etc.) carry a stable PascalCase `code` and a `docUrl: genexus://kb/tool-help/gotchas/<code>` the agent can fetch. Renames: `kb_charset_lossy` → `LintKbCharsetLossy`, `PreflightSpc0150` → `LintSpc0150ForEachAttributeWrite`.
- **`suggested_next_step` on errors (item 63).** `McpRouter.AttachSuggestedNextStep` is a pure pattern-match over error code/message. Wired into `DispatchCore`, the `KbResolutionException` catch, and preserved by `TrimErrorEnvelope`. Covers patch NoMatch/Ambiguous, visual-write failure, KB_AMBIGUOUS, spc0150.
- **`projection=minimal|standard|verbose` (item 64).** Optional parameter on `genexus_inspect` and `genexus_list_objects`. `minimal` returns name+type+lastUpdate; `standard` is the existing compact projection; `verbose` includes everything currently verbose-gated. Overrides `axiCompact` when set.
- **`genexus_sdk_probe` (item 14).** Reflective dump of the GeneXus SDK assembly surface (types, methods, properties) to `docs/sdk-probe/`. First call when hunting for SDK entry points to stop guessing names like `Form.GoTo`.
- **Auto-format Events on write (item 20).** `EventsFormatter` runs after patch context-match, before SDK save: normalizes indentation, aligns `=` columns in assignment blocks, collapses 3+ blank lines to 1. Opt out via `autoFormat=false`.
- **Universal `dryRun=true` (item 21).** Audited every edit-shaped tool: `genexus_edit`, `genexus_apply_pattern`, `genexus_create_object`, `genexus_create_popup`, `genexus_edit_and_build`, `genexus_history action=restore`, `genexus_undo`. All paths now return a unified diff of pre/post part bytes without persisting.
- **`genexus_lifecycle action=reorg_preview` (item 43).** Returns the list of `ALTER TABLE` / `CREATE TABLE` / `DROP COLUMN` statements the next reorg will execute, plus a summary (tables_added, columns_added, columns_dropped). Run before the destructive `action=reorg`.
- **Recipes catalog with examples (item 47).** Extended `genexus_recipe name=list` to return each recipe with a short description plus a concrete copy-pasteable args block for `genexus_recipe action=apply`. New `action=describe name=<recipe>` returns full prose docs.
- **`genexus_security action=scan_secrets`** plus extended `audit_gam` scanner (item 48 extension). Adds `JwtThreeSegment`, `PemBlock` (broadened from PRIVATE KEY → any KEY or CERTIFICATE), and `ConnectionStringWithUserAndPwd` (requires both User Id/UID *and* Password/Pwd to avoid false positives on parameterised templates). Each finding still carries `{severity, code, message, remediation}`.
- **`LintSqlInjection` lint (item 49).** `Helpers/SqlInjectionScanner.cs` scans `Events` and Procedure `Source` for `For each Where attr = &var.Concat(…)` / string-concat building Where / `&dyn = "where …" + &v` dynamic SQL builds. Non-blocking warning with line + suggested parametrised alternative.
- **Browser-driver fallback (item 70).** `Helpers/BrowserDriverDetector.cs` prefers `chrome-devtools-axi` on PATH, falls back to `npx playwright`, surfaces `BrowserDriverUnavailable` with both install hints when neither is found. Result cached; surfaced in `whoami.browserDriver`.
- **`notifyOnFailure=<webhook-url>` on `genexus_lifecycle build` (item 72).** POSTs `{kb, target, errors, errorsDetailedHead, jobId, durationSec}` when terminal state is `Failed` (not `PartialSuccess`). One-shot, no retries, no auth — paste a Slack/Discord webhook URL.
- **`GotchaWebFormTypedPropertyAutoRouted` warning + extended quirk table (item 77).** `WebFormTypedPropertyWriter` now records auto-routes via thread-static `AutoRouteRecord` registry; `WriteService` drains them after the write and surfaces `{from, to, element, controlId}` warnings so the agent learns the SDK silently renamed its property. New auto-routes: `OnEnterEvent`, `CaptionExpression` (alongside the existing `OnClickEvent` → `Event` for gxButton, `eventGX` for gxAttribute / gxImage).
- **`LintMasterPageIncompat` lint (item 80).** `Helpers/MasterPageCompatScanner.cs` scans WebForm writes for controls that depend on a different master page (`gxMessages`, `gxAttribute ControlType=ProgressIndicator`, `gxMenu`, `gxBreadcrumb`, `gxNotification` — conservative list). Warning surfaced before build.
- **`Services/BlameService.cs` foundation (item 88).** Service-only landing (not yet wired as a tool) for per-line git-blame attribution against the parts the SDK writes to disk. Returns `{line, commitHash, author, date, summary, snippet}` plus `code: "KbNotInGit"` when the KB is not a git repo.

### Earlier in v2.6.9 — initial wishlist sweep

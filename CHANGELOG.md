...
- **`genexus_list_objects` / `genexus_query` cold-start p99 47.5 s → 0.8 ms.** First list/query on a freshly-opened KB used to queue behind the worker's initial BulkIndex on the single STA thread, eating the full 60 s gateway timeout before returning an opaque "Gateway timeout" error. Now the gateway short-circuits to a structured `{status:"Indexing", code:"IndexNotReady", indexStatus, totalObjects, progress, hint}` envelope when the cached index state isn't yet "Ready". Subsequent calls hit the normal index path as soon as BulkIndex finishes. The worker-side ListService has a matching fast-fail for callers that bypass the gateway short-circuit.

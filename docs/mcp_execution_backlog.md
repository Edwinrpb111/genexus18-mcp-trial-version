...
- **`genexus_edit` html→layout Form-type transition.** Both `mode=patch` and `mode=full` fail with generic `"Visual write failed"` when only the `Form type` attribute changes. Workaround: full-replace with valid target-type XML. Error message should distinguish the unsupported transition from generic visual-write failure.
- **`genexus_search_source` doesn't index WebForm XML.** Tool scopes to source code parts (Procedure/DP source, WebPanel Events, Transaction source). Adding a `scope=WebForm` option would let agents harvest layout schema from existing panels before writing.
...
- **`genexus_apply_pattern reapply=true` can hang indefinitely.** v2.6.11 added projection-phase diagnostics, but the hang itself (long-running `CreatePatternInstanceWithTemplate + SetPatternApplyOnSave + ValidateAndSave`, possibly worsened when the IDE has the same panel open) isn't bounded. Needs a hard timeout (~60s) with clean abort, and an "IDE has the object open" detection before mutating.
